...
void part1() {
  ...
  while (getline(ifs, range) && range != "") {
    ...
    freshRanges.push_back({static_cast<uint64_t>(stoull(lower)),
                           static_cast<uint64_t>(stoull(upper))});
  }
  uint16_t freshIDs = 0;
  uint64_t ID;
  while (ifs >> ID) {
...
void part2() {
  ...
  for (uint16_t i = 1; i < freshRanges.size(); i++) {
    ...
    for (uint16_t j = 0; j < i; j++) {
      ...
      if (first < cleanFirst &&
          second > cleanSecond) { // hangs off on both sides
        freshRanges.erase(freshRanges.begin() + i);
        freshRanges.push_back({first, cleanFirst - 1});
        freshRanges.push_back({cleanSecond + 1, second});
        i--;
        break;
      } else if (first >= cleanFirst &&
                 second <= cleanSecond) { // fully overlaps within
        freshRanges.erase(freshRanges.begin() + i);
        i--;
        break;
      } else if (first <= cleanSecond &&
                 second > cleanSecond) { // overlaps on right side
        freshRanges[i].first = cleanSecond + 1;
      } else if (second >= cleanFirst &&
                 first < cleanFirst) { // overlaps on left side
